...
If you want to use your own CSS files, place the `.css` files in the `shared_css` or `css`
directory and add an entry to the front matter `stylesheets` with the filename body as key and
value as `true`.
...
If you import a `.zip` file created with the `npx tgweb-archive` command into TeamGenik,
the "stylesheets" section in `site.toml` and frontmatter will be ignored.
...
If you want to use your own JavaScript files, place the `.js` files in the `shared_js` or `js`
directory and add an entry to the front matter `javascripts` with the filename body as key and
value as `true`.
...
The above setting means that `main.js` in the `shared_js` directory or the `js` directory will be
used.

The `shared_js` directory is usually a symbolic link to `../../../shared_js`.
If the same file name exists in the `shared_js` directory and the `js` directory,
the one in the `js` directory takes precedence.

#### Notes

If you import a `.zip` file created with the `npx tgweb-archive` command into TeamGenik,
the "scripts" section in `site.toml` and frontmatter will be ignored.
...
The `<script>` elements within the `<head>` element are generated by the values ​​of properties
